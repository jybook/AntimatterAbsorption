...
void AliAnalysisTaskJulia::UserCreateOutputObjects()  {
    ...
    hdEdx_vs_momentum_negative = new TH2F ("hdEdx_vs_momentum_negative","",1000,-10.0,0.1,500,0,1000.0);
    fOutputList -> Add (hdEdx_vs_momentum_positive);
    fOutputList -> Add (hdEdx_vs_momentum_negative);

    //dE/dx vs. momentum both positive and negative
    hdEdx_vs_momentum = new TH2F ("hdEdx_vs_momentum", "", 1000, -10.0, 10.0, 500, 0, 1000.0);
    fOutputList -> Add (hdEdx_vs_momentum);
...
void AliAnalysisTaskJulia::UserExec(Option_t *)  {
    ...
    for (Int_t i=0 ; i<fAODevent->GetNumberOfTracks() ; i++)  {
        ...
        if (track -> Charge() < 0) hdEdx_vs_momentum_negative -> Fill (-p,dEdx);//minus sign added here
        hSigma_vs_momentum -> Fill (p, nsigmaTPC)
    }
